...
const imageMaxSize: number = 1900
...
export class ImageSlider extends React.Component<{}, { index: number }> {
  ...
  getImageSize = () => {
    ...
    return isVertical
      ? `h${window.innerHeight > imageMaxSize ? imageMaxSize : window.innerHeight}`
      : `w${window.innerWidth > imageMaxSize ? imageMaxSize : window.innerWidth}`
  }
      ...
      <div>
        {images.map((image, index) => (
          <div className={`bg-image ${this.state.index === index ? 'visible' : 'hidden'}`}
            style={{
              backgroundImage: `url(${image}=${this.getImageSize()})`,
